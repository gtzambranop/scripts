...
# [['character', num_repetitions], [...], ...]
def searchList(my_list, char):
    # Bandera que indica si crear o no una nueva sub-lista
    create = True
    if(len(my_list) == 0): # Lista vacía
        # se agrega una sublista con el caracter y 1
        my_list.append([char, 1])
    else:
...
def main():
	...
	with open("data.csv", 'r') as file:
		...
		for row in fl:
			...
			result = searchList(result, row[0])
		# Se ordena la lista
		result.sort()
		# Se imprime la lista
		printList(result)
